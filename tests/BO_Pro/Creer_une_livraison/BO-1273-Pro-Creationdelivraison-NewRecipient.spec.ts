...
test.describe(`BO-1273 - Creation de livraison pour nouveau destinataire @S6449c9c0`, () => {
  ...
  test(`Parcours complet - Nouveau destinataire - cas passant @T90ac7f75`, async ({ page }) => {
    await withCreationRetry(step1, async () => {
      ...
      await waitForDeliveryCreationAndRetry(page, orderInfo.reference, true, urlBeforeValidation);
    });
  });

  test(`Parcours complet - Nouveau destinataire sans email - cas passant @Tee21c457`, async ({ page }) => {
    await withCreationRetry(step1, async () => {
      const recipient = newRecipient();
      const orderInfo = generateOrderInformation();
  ...
  test(`Parcours complet - Nouveau destinataire sans prénom - cas passant @Td29a3dee`, async ({ page }) => {
    await withCreationRetry(step1, async () => {
      const recipient = newRecipient();
      const orderInfo = generateOrderInformation();
