...
test.describe(`BACK-4810 - Afficher les informations de la commande dans le détail de livraison @S774379e3`, () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await buildAndGotoDeliveryURL(page, (await createDeliveryAPI({ orderInfo: { reference: orderInfo.reference, amount: orderInfo.amount, additionalInfos: orderInfo.additionalInfos } })).id);
  });

  test(`Afficher la référence de la commande @Te1590dfe`, async () => {
    await expect(deliveryDetails.orderReference()).toHaveValue(orderInfo.reference);
  });

  test(`Afficher le montant de la commande @Td3ab673f`, async () => {
    await expect(deliveryDetails.orderAmount()).toHaveValue(`${orderInfo.amount},00`);
  });

  test(`Afficher le commentaire du point de retrait @Tc500db91`, async () => {
    await expect(deliveryDetails.storeComment()).toHaveValue(orderInfo.additionalInfos);
  });
});
